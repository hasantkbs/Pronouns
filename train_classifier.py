...
# EnhancedClassifier ve EarlyStopping sınıfları aynı kalıyor
class EnhancedClassifier(nn.Module):
    ...
    def forward(self, x):
        x = self.fc1(x); x = self.bn1(x); x = self.relu1(x); x = self.dropout1(x)
        x = self.fc2(x); x = self.bn2(x); x = self.relu2(x); x = self.dropout2(x)
        x = self.fc3(x)
        return x
...
def main():
    ...
    model = EnhancedClassifier(input_dim=config.EMBEDDING_DIM, hidden_dim1=512, hidden_dim2=256,output_dim=output_dim, dropout_rate=0.5)
